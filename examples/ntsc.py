...
NoDpdkCaseList = ["AdvancedFuzzing", "ScenarioDescrptionLanguage"]
...
class BaseCaseObject:
    ...
    def __init__(self, test_type):
        ...
        # general config
        if test_type in ["HttpCps", "HttpForceCps", "HttpCc", "HttpThroughput", "HttpsCps", "HttpsCc", "UdpPps"]:
            self.Monitor = "默认监控器对象Ping"
            self.Variate = "无"
            self.WebTestProjectName = "默认网络设备测试项目"
            self.FileObject = "默认156字节网页请求"

        elif test_type in ["HttpRequestFlood", "HttpMultipleRequest", "HttpRecursionRequest", "HttpConcurrentSlowRead",
                           "HttpConcurrentSlowRequest"]:
            self.Variate = "无"
            self.WebTestProjectName = "默认网络设备测试项目"
        ...
        # special config
        elif test_type in ["HttpThroughput"]:
            self.Monitor = "默认监控器对象Ping"
            self.Variate = "无"
            self.WebTestProjectName = "默认网络设备测试项目"
            self.FileObject = "默认512K字节网页请求"

        elif test_type in ["Rfc2544Throughput", "Rfc2544Latency", "Rfc2544LossRate", "Rfc2544BackToBack"]:
            # Monitoring of the tested equipment
            self.Monitor = "默认监控器对象Ping"
            # iMIX mixed frame length
            self.iMixName = "默认混合流量"

        elif test_type in ["UDPPayloadAttack"]:
            self.PacketPayloadOBJ = "默认handle_over_udp载荷模板"

        elif test_type in ["DnsServiceAttack", "DnsAmplificationAttack"]:
            self.Dns = "默认DNS服务攻击"
            self.Dnskey = "默认RSASHA256套件"

        elif test_type in ["HttpsFlood"]:
            self.SSLServerCertConfig = "默认RSA算法1024位私钥带密码证书套件"

        elif test_type in ["TcpSessionFlood"]:
            self.PacketPayloadOBJ = "默认handle_over_tcp载荷模板"

        elif test_type in ["HTTPSlowRequestFlood"]:
            self.Variate = "默认攻击Payload变量列表"
            self.WebTestProjectName = "默认Web攻击测试项目"
            self.FileObject = "默认SlowGet网页请求"

        # CaseObject is null
        elif test_type in ["TurboTcp", "Ipv4FragAttack", "IGMPv4SinglePacketAttack", "ARPv4SinglePacketAttack", "TCPSinglePacketAttack",
                           "UDPSinglePacketAttack", "SipSinglePacketAttack", "SSDPAttack", "NtpAmplificationAttack", "MemcachedAmplificationAttack",
                           "UnknownProtocolSinglePacketAttack", "MultiTypeFlood", "TCPWinnuke", "UnicastStorm", "BroadcastStorm"]:
            pass
...
class BaseClientProfiles:
    def __init__(self, test_type, dut_role="Gateway"):
        ...
        if test_type in ["HttpCps", "HttpForceCps", "HttpCc", "UDPPayloadAttack", "HttpRequestFlood", "HttpsFlood",
                         "HTTPSlowRequestFlood", "TcpSessionFlood", "HttpMultipleRequest", "HttpRecursionRequest",
                         "HttpConcurrentSlowRead", "HttpConcurrentSlowRequest", "MmsConnectStorm", "HttpsCps",
                         "HttpsCc", "HttpsThroughput", "SSLHandshake"]:
            self.Actions = {}
            self.RequestHeader = ["User-Agent: Firefox/41.0"]
            self.ClientCloseMode = "Reset"

        elif test_type in ["NtpAmplificationAttack", "SSDPAttack", "MemcachedAmplificationAttack"]:
            self.RandomLength = 8
            self.Actions = {}

        elif test_type in ["MultiTypeFlood", "TCPWinnuke", "UnicastStorm", "MulticastStorm", "BGPv4", "BGPv6"]:
            self.Actions = {}
...
class BaseServerProfiles:
    def __init__(self, test_type, dut_role="Gateway"):
        ...
        if test_type in ["HttpCps", "HttpForceCps", "HttpCc", "HttpThroughput", "Rfc2544Latency", "HttpRequestFlood",
                         "HTTPSlowRequestFlood", "TcpSessionFlood", "HttpMultipleRequest", "HttpRecursionRequest",
                         "HttpConcurrentSlowRead", "HttpConcurrentSlowRequest"]:
            self.ServerPort = "80"
            self.ResponseHeader = [
                "Server: nginx/1.9.5",
        ...
        if test_type in ["MmsConnectStorm"]:
            self.ServerPort = "102"
            self.ResponseHeader = [
                "Server: nginx/1.9.5",
                "Content-Type: text/html"
        ...
        if test_type in ["HttpsFlood", "HttpsCps", "HttpsCc", "HttpsThroughput", "SSLHandshake"]:
            self.ServerPort = "443"
            self.ResponseHeader = [
                "Server: nginx/1.9.5",
                "Content-Type: text/html"
        ...
        elif test_type in ["Ipv4FragAttack",  "TCPSinglePacketAttack", "MultiTypeFlood", "UnicastStorm", "MulticastStorm",
                           "VulnerabilityScanner", "WebScanner", "NetworkDiscovery"]:
            self.ServerPort = "80"

        elif test_type in ["UDPSinglePacketAttack"]:
            self.ServerPort = "69"

        elif test_type in ["UDPPayloadAttack"]:
            self.ServerPort = "2641"

        elif test_type in ["SipSinglePacketAttack"]:
            self.ServerPort = "5060"

        elif test_type in ["DnsServiceAttack"]:
            self.ServerPort = "53"

        elif test_type in ["SSDPAttack"]:
            self.ServerPort = "1900"

        elif test_type in ["DnsAmplificationAttack"]:
            self.ServerPort = "10000-65535"

        elif test_type in ["NtpAmplificationAttack"]:
            self.ServerPort = "123"

        elif test_type in ["MemcachedAmplificationAttack"]:
            self.ServerPort = "11211"

        elif test_type in ["TCPWinnuke"]:
            self.ServerPort = "139"

        elif test_type in ["AdvancedFuzzing"]:
            self.ServerPort = "5672"

        elif test_type in ["BGPv4", "BGPv6"]:
...
class HttpCps:
    ...
    class ClientProfiles(BaseClientProfiles):
        def __init__(self, test_type):
            super().__init__(test_type)

    class ServerProfiles(BaseServerProfiles):
        def __init__(self, test_type, dut_role="Gateway"):
            super().__init__(test_type, dut_role)
            if dut_role == "Server":
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpCps.ClientProfiles(test_type)
        self.ServerProfiles = HttpCps.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpForceCps:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpForceCps.ClientProfiles(test_type)
        self.ServerProfiles = HttpForceCps.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpCc:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpCc.ClientProfiles(test_type)
        self.ServerProfiles = HttpCc.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpThroughput:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpThroughput.ClientProfiles(test_type)
        self.ServerProfiles = HttpThroughput.ServerProfiles(test_type, dut_role=dut_role)
...
class UdpPps:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = UdpPps.ClientProfiles(test_type)
...
class TurboTcp:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = TurboTcp.CaseObject(test_type)
        self.ClientProfiles = TurboTcp.ClientProfiles(test_type)
...
class Rfc2544Throughput:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = Rfc2544Throughput.CaseObject(test_type)
        self.ClientProfiles = Rfc2544Throughput.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = Rfc2544Throughput.ServerProfiles(test_type, dut_role=dut_role)
...
class Rfc2544Latency:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = Rfc2544Latency.CaseObject(test_type)
        self.ClientProfiles = Rfc2544Latency.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = Rfc2544Latency.ServerProfiles(test_type, dut_role=dut_role)
...
class Rfc2544LossRate:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = Rfc2544LossRate.CaseObject(test_type)
        self.ClientProfiles = Rfc2544LossRate.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = Rfc2544LossRate.ServerProfiles(test_type, dut_role=dut_role)
...
class Rfc2544BackToBack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = Rfc2544BackToBack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = Rfc2544BackToBack.ServerProfiles(test_type, dut_role=dut_role)
...
class Ipv4FragAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = Ipv4FragAttack.CaseObject(test_type)
        self.ClientProfiles = Ipv4FragAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = Ipv4FragAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class ICMPSinglePacketAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = ICMPSinglePacketAttack.CaseObject(test_type)
...
class ARPv4SinglePacketAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = ARPv4SinglePacketAttack.CaseObject(test_type)
...
class TCPSinglePacketAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = TCPSinglePacketAttack.CaseObject(test_type)
        self.ClientProfiles = TCPSinglePacketAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = TCPSinglePacketAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class UDPSinglePacketAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = UDPSinglePacketAttack.CaseObject(test_type)
        self.ClientProfiles = UDPSinglePacketAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = UDPSinglePacketAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class UDPPayloadAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = UDPPayloadAttack.CaseObject(test_type)
        self.ClientProfiles = UDPPayloadAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = UDPPayloadAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class SipSinglePacketAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = SipSinglePacketAttack.CaseObject(test_type)
        self.ClientProfiles = SipSinglePacketAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = SipSinglePacketAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class DnsServiceAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = DnsServiceAttack.CaseObject(test_type)
        self.ClientProfiles = DnsServiceAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = DnsServiceAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class DnsAmplificationAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = DnsAmplificationAttack.CaseObject(test_type)
        self.ClientProfiles = DnsAmplificationAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = DnsAmplificationAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class SSDPAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = SSDPAttack.CaseObject(test_type)
        self.ClientProfiles = SSDPAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = SSDPAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class NtpAmplificationAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = NtpAmplificationAttack.CaseObject(test_type)
        self.ClientProfiles = NtpAmplificationAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = NtpAmplificationAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class MemcachedAmplificationAttack:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = MemcachedAmplificationAttack.CaseObject(test_type)
        self.ClientProfiles = MemcachedAmplificationAttack.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = MemcachedAmplificationAttack.ServerProfiles(test_type, dut_role=dut_role)
...
class UnknownProtocolSinglePacketAttack:
    ...
    def __init__(self, test_type):
        ...
        self.CaseObject = UnknownProtocolSinglePacketAttack.CaseObject(test_type)
...
class HttpRequestFlood:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpRequestFlood.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpRequestFlood.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpsFlood:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = HttpsFlood.CaseObject(test_type)
        self.ClientProfiles = HttpsFlood.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpsFlood.ServerProfiles(test_type, dut_role=dut_role)
...
class HTTPSlowRequestFlood:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = HTTPSlowRequestFlood.CaseObject(test_type)
        self.ClientProfiles = HTTPSlowRequestFlood.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HTTPSlowRequestFlood.ServerProfiles(test_type, dut_role=dut_role)
...
class MultiTypeFlood:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = MultiTypeFlood.CaseObject(test_type)
        self.ClientProfiles = MultiTypeFlood.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = MultiTypeFlood.ServerProfiles(test_type, dut_role=dut_role)
...
class TcpSessionFlood:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = TcpSessionFlood.CaseObject(test_type)
        self.ClientProfiles = TcpSessionFlood.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = TcpSessionFlood.ServerProfiles(test_type, dut_role=dut_role)
...
class TCPWinnuke:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = TCPWinnuke.CaseObject(test_type)
        self.ClientProfiles = TCPWinnuke.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = TCPWinnuke.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpMultipleRequest:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpMultipleRequest.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpMultipleRequest.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpRecursionRequest:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpRecursionRequest.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpRecursionRequest.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpConcurrentSlowRead:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpConcurrentSlowRead.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpConcurrentSlowRead.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpConcurrentSlowRequest:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpConcurrentSlowRequest.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpConcurrentSlowRequest.ServerProfiles(test_type, dut_role=dut_role)
...
class UnicastStorm:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = UnicastStorm.CaseObject(test_type)
        self.ClientProfiles = UnicastStorm.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = UnicastStorm.ServerProfiles(test_type, dut_role=dut_role)
...
class BroadcastStorm:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = BroadcastStorm.CaseObject(test_type)
...
class MulticastStorm:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.CaseObject = MulticastStorm.CaseObject(test_type)
        self.ClientProfiles = MulticastStorm.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = MulticastStorm.ServerProfiles(test_type, dut_role=dut_role)
...
class MmsConnectStorm:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = MmsConnectStorm.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = MmsConnectStorm.ServerProfiles(test_type, dut_role=dut_role)
...
class VulnerabilityScanner:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ServerProfiles = VulnerabilityScanner.ServerProfiles(test_type, dut_role=dut_role)
...
class WebScanner:
    ...
    def __init__(self, dut_role, test_type):
        self.Loads = WebScanner.Loads()
        self.CaseObject = WebScanner.CaseObject()
        self.ClientProfiles = WebScanner.ClientProfiles()
        self.ServerProfiles = WebScanner.ServerProfiles(test_type, dut_role=dut_role)
...
class NetworkDiscovery:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ServerProfiles = NetworkDiscovery.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpsCps:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpsCps.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpsCps.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpsCc:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpsCc.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpsCc.ServerProfiles(test_type, dut_role=dut_role)
...
class HttpsThroughput:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = HttpsThroughput.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = HttpsThroughput.ServerProfiles(test_type, dut_role=dut_role)
...
class SSLHandshake:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = SSLHandshake.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = SSLHandshake.ServerProfiles(test_type, dut_role=dut_role)
...
class AdvancedFuzzing:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ServerProfiles = AdvancedFuzzing.ServerProfiles(test_type, dut_role=dut_role)
...
class BGPv4:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = BGPv4.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = BGPv4.ServerProfiles(test_type, dut_role=dut_role)
...
class BGPv6:
    ...
    def __init__(self, dut_role, test_type):
        ...
        self.ClientProfiles = BGPv6.ClientProfiles(test_type, dut_role=dut_role)
        self.ServerProfiles = BGPv6.ServerProfiles(test_type, dut_role=dut_role)
...
class ScenarioDescrptionLanguage:
    class Loads:
        def __init__(self):
            self.SendWaitTime = 0
            self.SendNumCyles = 1
    ...
    def __init__(self):
        self.Loads = ScenarioDescrptionLanguage.Loads()
        self.CaseObject = ScenarioDescrptionLanguage.CaseObject()
        self.ClientProfiles = ScenarioDescrptionLanguage.ClientProfiles()
        self.ServerProfiles = ScenarioDescrptionLanguage.ServerProfiles()
...
class PortConfig:
    def __init__(self, port_name, port_side, case_config):
        ...
        if port_side == "client":
            ...
            elif test_type in ["ScenarioDescrptionLanguage"]:
        ...
        if test_type not in NoDpdkCaseList:
...
class TestCase:
    ...
    def _handle_interface(self, *args):
        ...
        if self.test_type not in NoDpdkCaseList:
...
class TestFactory:
    ...
    def create(test_type, dut_role):
        if test_type == "HttpCps":
            ...
            return TurboTcp(dut_role, test_type)
        elif test_type == "Rfc2544Throughput":
            return Rfc2544Throughput(dut_role, test_type)
        elif test_type == "Rfc2544Latency":
            return Rfc2544Latency(dut_role, test_type)
        elif test_type == "Rfc2544LossRate":
            return Rfc2544LossRate(dut_role, test_type)
        elif test_type == "Rfc2544BackToBack":
            return Rfc2544BackToBack(dut_role, test_type)

        elif test_type == "Ipv4FragAttack":
            return Ipv4FragAttack(dut_role, test_type)
        elif test_type == "ICMPSinglePacketAttack":
            return ICMPSinglePacketAttack(dut_role, test_type)
        elif test_type == "IGMPv4SinglePacketAttack":
            return IGMPv4SinglePacketAttack(dut_role, test_type)
        elif test_type == "ARPv4SinglePacketAttack":
            return ARPv4SinglePacketAttack(dut_role, test_type)
        elif test_type == "TCPSinglePacketAttack":
            return TCPSinglePacketAttack(dut_role, test_type)
        elif test_type == "UDPSinglePacketAttack":
            return UDPSinglePacketAttack(dut_role, test_type)
        elif test_type == "UDPPayloadAttack":
            return UDPPayloadAttack(dut_role, test_type)
        elif test_type == "SipSinglePacketAttack":
            return SipSinglePacketAttack(dut_role, test_type)
        elif test_type == "DnsServiceAttack":
            return DnsServiceAttack(dut_role, test_type)
        elif test_type == "DnsAmplificationAttack":
            return DnsAmplificationAttack(dut_role, test_type)
        elif test_type == "SSDPAttack":
            return SSDPAttack(dut_role, test_type)
        elif test_type == "NtpAmplificationAttack":
            return NtpAmplificationAttack(dut_role, test_type)

        elif test_type == "MemcachedAmplificationAttack":
            return MemcachedAmplificationAttack(dut_role, test_type)
        elif test_type == "UnknownProtocolSinglePacketAttack":
            return UnknownProtocolSinglePacketAttack(test_type)
        elif test_type == "HttpRequestFlood":
            return HttpRequestFlood(dut_role, test_type)
        elif test_type == "HttpsFlood":
            return HttpsFlood(dut_role, test_type)
        elif test_type == "HTTPSlowRequestFlood":
            return HTTPSlowRequestFlood(dut_role, test_type)
        elif test_type == "MultiTypeFlood":
            return MultiTypeFlood(dut_role, test_type)
        elif test_type == "TcpSessionFlood":
            return TcpSessionFlood(dut_role, test_type)
        elif test_type == "TCPWinnuke":
            return TCPWinnuke(dut_role, test_type)
            ...
            return UnicastStorm(dut_role, test_type)
        elif test_type == "BroadcastStorm":
            return BroadcastStorm(dut_role, test_type)
        elif test_type == "MulticastStorm":
            return MulticastStorm(dut_role, test_type)
            ...
            return MmsConnectStorm(dut_role, test_type)
        elif test_type == "LLDPStormTest":
            return LLDPStormTest(dut_role)
        elif test_type == "VulnerabilityScanner":
            return VulnerabilityScanner(dut_role, test_type)
        elif test_type == "WebScanner":
            return WebScanner(dut_role, test_type)
        elif test_type == "NetworkDiscovery":
            return NetworkDiscovery(dut_role, test_type)
            ...
            return SSLHandshake(dut_role, test_type)
        elif test_type == "AdvancedFuzzing":
            return AdvancedFuzzing(dut_role, test_type)

        elif test_type == "BGPv4":
            return BGPv4(dut_role, test_type)
        elif test_type == "BGPv6":
            return BGPv6(dut_role, test_type)

        elif test_type == "ScenarioDescrptionLanguage":
            return ScenarioDescrptionLanguage()
